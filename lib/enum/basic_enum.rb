...
class MyEnumeration
  ...
  def number_of_floats_or_fixnums
    collection.select { |x| x.is_a? (Numeric)}.count
  end
  ...
  def find_first_awesome_person
    collection.find { |x| x[:awesome] == true}
  end
  ...
  def sum_of_experiences
    collection.map { |element| element[:years_experience]}.inject(:+)
  end
  ...
  def longest_element_name_using_inject
    collection.max_by { |x| x[:name].length }
  end
